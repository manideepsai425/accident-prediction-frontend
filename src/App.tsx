...
function Badge({ children }: { children: ReactNode }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/10 px-3 py-1 text-xs font-medium text-slate-100 backdrop-blur-sm">
      {children}
    </span>
  )
}
...
export default function App() {
  ...
  const statusLabel = !healthChecked ? 'Connecting…' : health?.status === 'ok' ? 'Model ready' : 'API offline'
  const statusOk = healthChecked && health?.status === 'ok'

  return (
    <div className="min-h-screen bg-canvas">
      {/* ======================= Dark gradient header ======================= */}
      <header className="relative overflow-hidden bg-gradient-to-br from-navy to-navy-light pb-24 pt-6">
        <div className="mx-auto flex max-w-6xl flex-col gap-5 px-4 sm:px-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs leading-tight text-slate-400">ML Risk Scoring · Dijkstra Routing · Telangana</p>
              </div>
            </div>
            <nav className="flex rounded-full bg-white/10 p-1 text-sm font-medium backdrop-blur-sm">
              <button
                onClick={() => setPage('planner')}
                className={`rounded-full px-3.5 py-1.5 transition-colors ${page === 'planner' ? 'bg-white text-navy' : 'text-slate-300 hover:text-white'}`}
              >
                Planner
              </button>
              <button
                onClick={() => setPage('analytics')}
                className={`rounded-full px-3.5 py-1.5 transition-colors ${page === 'analytics' ? 'bg-white text-navy' : 'text-slate-300 hover:text-white'}`}
              >
                Analytics
              </button>
            </nav>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Badge>{health?.road_segments_loaded ?? '—'} Segments</Badge>
            <span
              className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium ${
                statusOk ? 'bg-brand-mist text-brand-deep' : 'bg-white/10 text-slate-300'
              }`}
            >
              <span className={`h-1.5 w-1.5 rounded-full ${statusOk ? 'animate-pulse bg-brand' : 'bg-slate-400'}`} />
              {statusLabel}
            </span>
          </div>
        </div>
      ...
      <div className="mx-auto -mt-16 max-w-6xl px-4 sm:px-6">
        {page === 'planner' && (
          <motion.div
            ...
            className="rounded-2xl bg-white p-4 shadow-xl shadow-navy/10 sm:p-5"
          >
            ...
            <form onSubmit={handleFindRoute} className="flex flex-col gap-3">
              <div className="flex items-center gap-2">
                <div className="flex flex-1 flex-col gap-2">
                  <input
                    ...
                    className="rounded-xl border border-slate-200 px-3 py-2.5 text-sm focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand-mist"
                  />
                  <input
                    list="place-list"
                    value={destName}
                ...
                <button
                  ...
                  className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border border-slate-200 text-slate-500 transition-colors hover:border-brand hover:bg-brand-mist hover:text-brand-deep"
                >
                  ⇅
                </button>
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-shrink-0 items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium transition-colors ${
                        role ? 'border-brand bg-brand-mist text-brand-deep' : 'border-slate-200 bg-white text-slate-600 hover:border-brand-mist'
                      }`}
                    >
                      {p.name}
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 rounded-xl bg-brand py-3 text-sm font-semibold text-white transition-colors hover:bg-brand-deep disabled:opacity-60"
              >
                {routeLoading ? (
                  <>
        ...
        {page === 'planner' ? (
          <div ref={resultsRef} className="scroll-mt-4">
                ...
                >
                  ...
                  <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-card">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <div className="flex flex-wrap items-center gap-2 text-xs">
                        <span className="rounded-full bg-slate-100 px-2.5 py-1 font-medium text-slate-600">{weather}</span>
                        <span className="rounded-full bg-slate-100 px-2.5 py-1 font-medium text-slate-600">{preferredTime}</span>
                  ...
                  <div className={`grid grid-cols-1 gap-5 ${mapVisible ? 'lg:grid-cols-5' : ''}`}>
                        ...
                        >
                          ...
                          <div className="absolute right-3 top-3 z-[2100] flex gap-2">
                            <button
                              ...
                              className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold shadow-card transition-colors ${
                            ...
                            {!mapFullscreen ? (
                              <button
                                ...
                                className="rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-ink shadow-card transition-colors hover:bg-brand-mist"
                              >
                                ⛶ Full screen
                              </button>
                            ) : (
                          ...
                          {checkPoint && !mapFullscreen && (
                            <div className="absolute bottom-3 left-3 right-3 z-[2100] rounded-xl bg-white/95 p-3 shadow-card backdrop-blur-sm">
                              <div className="flex items-center justify-between gap-2">
                                ...
                                <button
                                  ...
                                  className="flex items-center gap-1.5 rounded-full bg-ink px-3 py-1 text-xs font-semibold text-white disabled:opacity-60"
                                >
                              ...
                              {checkResult && (
                                <div className="mt-2 flex items-center gap-2">
                                  ...
                                  <p className="text-xs text-slate-500">{checkResult.explanation}</p>
                                </div>
                              )}
                            </div>
                          )}
                    ...
                    <div className={mapVisible ? 'lg:col-span-3' : ''}>
                      <div className="flex items-center justify-between">
                        <div className="relative flex gap-1 rounded-full bg-slate-100 p-1 text-sm font-medium">
                          {(
                            [
                              ['routes', '✅ Routes'],
                              ['details', '🗺️ Details'],
                            ...
                            <button
                              ...
                              className="relative rounded-full px-3 py-1.5 text-slate-500 transition-colors data-[active=true]:text-brand-deep"
                        ...
                        <button
                          ...
                          className="hidden rounded-full border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-500 transition-colors hover:border-brand hover:text-brand-deep lg:inline-block"
                        >
                      ...
                      <AnimatePresence mode="wait">
                        ...
                        >
                                  ...
                                  <tbody>
                                    {allRoutes.map((r) => (
                                      ...
                                      >
                                        ...
                                        <td className="px-4 py-3 text-slate-600">{r.total_distance_km}</td>
                                        <td className="px-4 py-3 text-slate-600">{Math.round(r.est_travel_time_min)}</td>
                                ...
                                {riskiestSegments.map((s) => (
                                  <div key={s.segment_id}>
                                    <div className="mb-1 flex items-center justify-between gap-2 text-xs">
                                      ...
                                      <div className="flex items-center gap-2">
                                        <span className="text-slate-400">{s.risk_score.toFixed(2)}</span>
                          ...
                          {activeTab === 'explain' && selectedRoute && (
                            <div className="rounded-2xl border border-slate-100 bg-white p-4 shadow-card">
                              <SectionLabel>Risk explanation — {selectedRoute.label}</SectionLabel>
                              {factorsLoading && (
                              ...
                              <div className="flex flex-col gap-4">
                                ...
                                {selectedRoute.segments.every((s) => s.risk_level === 'Low') && (
                                  <p className="text-xs text-slate-400">No elevated-risk segments on this route — nothing to flag.</p>
                                )}
                              </div>
                            </div>
                          )}
            ...
            {!routeResult && !routeLoading && (
              <div className="flex flex-col items-center justify-center gap-2 rounded-2xl border border-dashed border-slate-200 py-16 text-center">
                <p className="text-sm font-medium text-slate-500">Pick an origin and destination above to see route options.</p>
                <p className="text-xs text-slate-400">Results — safest route, comparisons, and risk detail — will appear here.</p>
              </div>
            )}
          </div>
        ) : (
...
function RouteAccordion({ route, isSelected, onSelect }: { route: RouteOption; isSelected: boolean; onSelect: () => void }) {
  ...
  return (
    <div className={`rounded-2xl border p-4 shadow-card transition-colors ${isSelected ? 'border-brand bg-brand-mist/30' : 'border-slate-100 bg-white'}`}>
      ...
      <button onClick={() => setExpanded((v) => !v)} className="mt-3 text-xs font-semibold text-brand-deep">
      ...
      <AnimatePresence initial={false}>
        {expanded && (
          ...
          >
            <ol className="mt-3 flex flex-col gap-2 border-t border-slate-100 pt-3">
              {route.segments.map((s, i) => (
                <li key={s.segment_id} className="flex items-center justify-between gap-2 text-xs">
                  <span className="text-slate-500">
                    {i + 1}. {s.road_name} <span className="text-slate-400">· {s.length_km.toFixed(1)} km</span>
                  </span>
                  <RiskPill level={s.risk_level} />
                </li>
              ))}
